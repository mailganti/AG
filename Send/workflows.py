...
# ============================
# FIX: DEPENDENCY FUNCTIONS
# ============================

# This function validates the token and returns the actor (token user's ID/name)
def require_role(roles: List[str],x_admin_token: str = Header(...')):
    ...
    # Uses Header dependency to read the X-Token from the request
    #def wrapper(x_token: Optional[str] = Header(None, alias="X-Token")) -> str:
...
eef require_role_actor(roles: List[str]):
    # This is the actual dependency injection function
    return Depends(require_role(roles))

# ============================
# ROUTES (FIXED TO USE ACTOR DEPENDENCY)
# ============================

@router.post("/")
def create_workflow(body: WorkflowCreate, actor: str = require_role_actor(["admin", "requestor"])):
    # FIX: Using 'actor' which is the authenticated user ID/name
    db = get_db()
    ...
    # FIX: Logging the actual actor
    db.add_audit(wid, "created", actor, note=body.reason) 
    return {"workflow_id": wid}

@router.get("/")
def list_workflows(limit: int = 100, actor: str = require_role_actor(["admin", "approver", "viewer"])):
    # FIX: Using 'actor' dependency for validation
    db = get_db()
    return db.list_workflows(limit)

@router.get("/{workflow_id}")
def get_workflow(workflow_id: str, actor: str = require_role_actor(["admin", "approver", "viewer"])):
    # FIX: Using 'actor' dependency for validation
    db = get_db()
...
def get_audit(workflow_id: str, actor: str = require_role_actor(["admin", "approver", "viewer"])):
    # FIX: Using 'actor' dependency for validation
    db = get_db()
    return {"audit": db.get_audit(workflow_id)}

@router.post("/{workflow_id}/approve")
def approve_workflow(workflow_id: str, body: WorkflowApprove, actor: str = require_role_actor(["admin", "approver"])):
    # FIX: Using 'actor' dependency for validation and audit log
    db = get_db()
    wf = db.get_workflow(workflow_id)
    if not wf:
        raise HTTPException(status_code=404, detail="not found")
...
def deny_workflow(workflow_id: str, body: WorkflowApprove, actor: str = require_role_actor(["admin", "approver"])):
    # FIX: Using 'actor' dependency for validation and audit log
    db = get_db()
    wf = db.get_workflow(workflow_id)
    if not wf:
...
def execute_workflow(workflow_id: str, actor: str = require_role_actor(["admin"])):
    # FIX: Using 'actor' dependency for validation and audit log
    db = get_db()
    wf = db.get_workflow(workflow_id)
    if not wf:
